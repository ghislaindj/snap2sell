class MepGenerator < Rails::Generators::NamedBase
  ...
  def generate_deploy_keys
    ...
    github.repos.keys.create(user: 'milkysocial', 
                            repo: app_name,
  ...
  private
    ...
    def client_url
      if options.client_url.present?
        ...
        "#{app_name.gsub('_', '-')}-staging.milky.fr"
      else
        "#{app_name.gsub('_', '-')}.milky.fr"
      end
    end
    def github
      unless @github.present?
        ...
        #   return github
        # end
        @github = Github.new oauth_token: "da48a92710e9fedd3dd1e36e55e3c25d77f4ee64", org: 'milkysocial'
      end
      @github
    end
end